...
>
	...
	>
		<div class="flex items-center">
			<h2 class="mx-auto w-full indent-10 font-bold capitalize text-white">Pesquisar</h2>
			<button
				onclick={() => (search = '')}
				...
				{#each data as song}
					...
					>
						...
						<div class="flex flex-col">
							...
							<p class="font-thin text-xs">{song.artists[0].name} <span class="opacity-40">{song.album.name} - {song.album.release_date}</span></p>
						</div>
					</a>
				{/each}
			{:else}
