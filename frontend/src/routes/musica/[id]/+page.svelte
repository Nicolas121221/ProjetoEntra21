<script>
	...
	onMount(async () => {
		try {
			let res = await fetch(`http://localhost:3000/spotify/song/${page.params.id}`);
			if (res) data = await res.json();
			res = await fetch(`http://localhost:3000/spotify/artist/${data.artists[0].id}`);
...
{#if data && artist}
	...
	>
		<h1 class="border-b border-blue-400/50 pb-4 text-center text-2xl font-semibold">
			Pesquisas recentes
		</h1>
	</aside>
	<main class="h-screen w-full pt-10 text-white">
		<section class="ml-70 h-90 flex py-20">
			<div class=" pl-40">
				<img src={data.album.images[0].url} alt="" class="aspect-square size-80 blur-3xl" />
				<img src={data.album.images[0].url} alt="" class="relative -mt-80 aspect-square size-80" />
			</div>
			<div class="text-shadow-xs text-shadow-black pl-10 pt-48">
				<h5 class="text-2xl">{data.album.name}</h5>
				<div class="mt-3 flex gap-2">
					<img src={artist.images[0].url} class="inline-block size-7 rounded-full shadow" alt="" />
					<h4 class="inline-block text-2xl font-semibold">
						{data.artists[0].name} -
						<span class="text-xl opacity-60">{data.album.release_date}</span>
					</h4>
				</div>
				<h1 class="text-5xl font-bold">{data.name}</h1>
				<div class="absolute right-10 -mt-5 flex justify-center items-center">
					<a href={artist.external_urls.spotify} target="_blank">
						<img
							src="https://static.vecteezy.com/system/resources/previews/031/737/196/original/spotify-icon-spotify-social-media-logo-free-png.png"
							alt="Logo do spotify"
							class="size-10"
						/>
					</a>
		...
		<section
			class="ml-70 mt-30 min-h-screen rounded-2xl border border-blue-400/50 bg-gray-950/80 p-10"
		>
		<h3 class="font-bold text-5xl indent-28 mt-5">Cifra</h3>
			<pre class="ml-30">
				{#each chords as part}
